...
if not os.path.exists('data.json'):
    ...
    with open('data.json') as infile:
        ...
        os.makedirs('data_fid', exist_ok=True)
        for t in ['train', 'val', 'test']:
            data_split = all_data[t]
            with open('data_fid/{}.source'.format(t), 'w') as source, open('data_fid/{}.target'.format(t), 'w') as target:
                for data in data_split:
                    fid = '\t'.join(['{} <SEP> {}'.format(data['Q'], r) for r in data['R']])
                    source.write('{}\n{}\n'.format(fid, fid))
